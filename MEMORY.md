...
- **Assembled lesson pages:** `lessons/kids.html` + `lessons/adults.html` are GENERATED by
  `lessons/build.mjs` from the markdown copy + widget configs + images (don't hand-edit the
  HTML; edit sources and re-run `node lessons/build.mjs`). Polished with `lesson.css`
  (kids = spark-gradient completion cards; adults = clean checkpoints), scroll progress bar.
