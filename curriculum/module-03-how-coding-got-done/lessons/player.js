...
(function () {
  ...
  var MODULE = "module-03-how-coding-got-done";
  ...
  // Module 3 launch floor: missions stay locked until this date no matter when
  // a learner first opens it. Self-expires (anyone who first plays after this
  // date anchors to their own start). Module 1 launches 2026-07-06; Module 3
  // follows the next week. Change this one line to move Module 3's launch.
  var LAUNCH_FLOOR = "2026-07-20";
  ...
  function showLock(part) {
    ...
    var preLaunch = part.n === 1;  // Module 3 itself hasn't opened yet
    stage.innerHTML =
      '<div class="beat t-lock revealed"><div class="lock-card">' +
      '<div class="lock-ico">' + (preLaunch ? "🚀" : "🔒") + '</div>' +
      '<h2>' + (preLaunch ? "Module 3 opens " + whenH : "Part " + part.n + " unlocks " + whenH) + '!</h2>' +
      (preLaunch
        ? '<p>Nice — you found the next adventure! 🚀 <b>Module 3: How Coding Got Done</b> is fueled up and opens on launch day. Keep building in Module 1 and watch for it!</p>'
    ...
    hint.textContent = "";
    label.textContent = preLaunch ? "Module 3 · locked 🚀" : ("Part " + part.n + " · locked");
  }
  ...
  function onDbReady() {
    ...
    if (st && st.kind === "beat" && st.b.type === "complete" && st.m && st.m.n) {
      d.awardBadge(TRACK + "-mod3-" + st.m.n, { module: MODULE, track: TRACK });
    }
  ...
  function render() {
    ...
    if (step.kind === "title") {
      ...
      if (type === "complete") {
        ...
        var bart = (window.CTFBadge && window.CTFBadge.renderKey && n) ? window.CTFBadge.renderKey("kids-mod3-" + n) : "";
        var bname = (window.CTFBadge && window.CTFBadge.nameFor && n) ? window.CTFBadge.nameFor("kids-mod3-" + n) : "";
        ...
        confetti();
        var dd = db();
        if (dd && n) { dd.awardBadge(TRACK + "-mod3-" + n, { module: MODULE, track: TRACK }); dd.logEvent("mission_complete", { module: MODULE, track: TRACK, n: n }); }
        /* pet reward is Module 2 only */
  ...
  // ---- Module 3 sidekick pet picker (full-screen, first entry) --------------
  ...
  // shown ONCE at the start of Module 3: a preview of the reward you can earn
  ...
  function petPreview() {
    ...
    ov.innerHTML =
      '<div class="pp-head">🎁 Finish Module 3 to earn a sidekick! 🎁</div>' +
  ...
  function petPicker() {
    ...
    ov.innerHTML =
      '<div class="pp-head">🎉 You finished Module 3! 🎉</div>' +
