...
(function () {
  ...
  // =========================================================================
  // MODULE 3 GAMES — how coding got done: write code by hand, hunt bugs, pick
  // the right language, prompt an AI, drive the agent loop, direct a team.
  // =========================================================================

  // CODE BY HAND (Mission 3) — reorder scrambled code lines; Run; one line out
  // of place breaks it. Feel how careful by-hand coding was.
  function renderCodeHand(root, cfg, id){
    var LEVELS = cfg.levels || [
      { title:'Make toast', goal:'A simple morning routine — written as code!', lines:['Open the bread bag','Put bread in the toaster','Push the lever down','Wait until it’s golden','Spread on the butter'] },
      { title:'Greet the user', goal:'A real little program that says hello.', lines:['Ask the user for their name','Save the answer in a box called name','Print "Hello, " + name'] },
      { title:'Draw a square', goal:'Tell the turtle to draw four sides.', lines:['Put the pen down','Repeat 4 times:','   Move forward 100 steps','   Turn right 90 degrees','Lift the pen up'] }
    ];
    fsGame(root, cfg, id, { ico:'⌨️', title: cfg.title||'Code by Hand',
      blurb:'Back when humans wrote every line by hand, the <b>order</b> had to be perfect. Each program’s lines are scrambled. Put them in order, then hit Run. One line out of place and it breaks — fix it and run again!',
      playLabel:'▶  Start coding', wrapClass:'g-code',
      thanks: cfg.thanks || 'One line out of place breaks everything — that’s by-hand coding. ⌨️',
      play:function(host, api){
        var li, order, correct;
        function start(){ li=0; load(); }
        function load(){
          if(li>=LEVELS.length) return win();
          correct = LEVELS[li].lines.slice();
          order = shuffle(correct.slice());
          if(order.join('|')===correct.join('|')) order.reverse();
          render();
        }
        function render(){
          var L=LEVELS[li];
          host.innerHTML='<div class="ch-bg"></div>'+
            '<div class="ch-top"><span>Program '+(li+1)+' / '+LEVELS.length+'</span><span class="ch-name">'+esc(L.title)+'</span></div>'+
            '<div class="ch-goal">🎯 '+esc(L.goal)+'</div>'+
            '<div class="ch-editor">'+order.map(function(ln,i){
              return '<div class="ch-line"><span class="ch-num">'+(i+1)+'</span><code>'+esc(ln)+'</code>'+
                '<span class="ch-mv"><button class="ch-up" data-i="'+i+'"'+(i===0?' disabled':'')+'>▲</button>'+
                '<button class="ch-dn" data-i="'+i+'"'+(i===order.length-1?' disabled':'')+'>▼</button></span></div>';
            }).join('')+'</div>'+
            '<div class="ch-msg"></div>'+
            '<button class="imm-btn ch-run" data-run>▶ Run the program</button>';
          host.querySelectorAll('.ch-up').forEach(function(b){ b.addEventListener('click',function(){ mv(+b.getAttribute('data-i'),-1); }); });
          host.querySelectorAll('.ch-dn').forEach(function(b){ b.addEventListener('click',function(){ mv(+b.getAttribute('data-i'),1); }); });
          host.querySelector('[data-run]').addEventListener('click', run);
        }
        function mv(i,d){ var j=i+d; if(j<0||j>=order.length) return; var t=order[i]; order[i]=order[j]; order[j]=t; render(); }
        function run(){
          var fw=-1; for(var i=0;i<correct.length;i++){ if(order[i]!==correct[i]){ fw=i; break; } }
          var lines=host.querySelectorAll('.ch-line'); lines.forEach(function(l){ l.classList.remove('err','ok'); });
          var msg=host.querySelector('.ch-msg');
          if(fw<0){ lines.forEach(function(l){ l.classList.add('ok'); }); msg.innerHTML='<span class="ok">✅ It runs perfectly!</span>'; li++; setTimeout(load,1100); }
          else { lines[fw].classList.add('err'); msg.innerHTML='<span class="bad">💥 Error near line '+(fw+1)+' — that step isn’t in the right spot yet. Rearrange and run again.</span>'; }
        }
        function win(){ host.innerHTML=immResult('🏅','Every program runs!', true, '<p>You ordered each line by hand — exactly how coders worked for decades. See how <b>one line out of place breaks everything</b>? That’s why it was slow, careful work. Next: what happens when there’s a mistake — bugs!</p>'); bindRes(host, api, start); api.done(); }
        start();
      }});
  }

  // BUG HUNT (Mission 4) — tap the one wrong token to fix each line. 3 hearts.
  function renderBugHunt(root, cfg, id){
    var ROUNDS = cfg.rounds || [
      { tokens:['prnt','(','"Hi!"',')'], bug:0, fix:'print', hint:'A word is spelled wrong.' },
      { tokens:['if','score','=','100',':'], bug:2, fix:'==', hint:'Checking if two things are equal needs a DOUBLE sign.' },
      { tokens:['name','=','inputt','(',')'], bug:2, fix:'input', hint:'One word has an extra letter.' },
      { tokens:['total','=','5','x','3'], bug:3, fix:'*', hint:'Computers multiply with a star, not an x.' },
      { tokens:['print','(','scor',')'], bug:2, fix:'score', hint:'A word is missing a letter.' }
    ];
    fsGame(root, cfg, id, { ico:'🐛', title: cfg.title||'Bug Hunt',
      blurb:'Every coder’s job: hunt down the <b>bug</b> — a tiny mistake that stops the code. Each line has exactly ONE bug. Tap the part that’s wrong to fix it. You have 3 hearts, and they get sneakier each round!',
      playLabel:'▶  Start hunting', wrapClass:'g-bug',
      thanks: cfg.thanks || 'Bugs are normal — finding them is the real skill. 🐛',
      play:function(host, api){
        var ri, hearts, queue;
        function start(){ ri=0; hearts=3; queue=shuffle(ROUNDS); render(); }
        function render(){
          if(ri>=queue.length) return win();
          var r=queue[ri];
          host.innerHTML='<div class="bug-bg"></div>'+
            '<div class="bug-top"><span>Bug '+(ri+1)+' / '+queue.length+'</span><span class="bug-hearts">'+'❤️'.repeat(hearts)+'🖤'.repeat(3-hearts)+'</span></div>'+
            '<div class="bug-instr">Tap the part of this code that’s <b>wrong</b>:</div>'+
            '<div class="bug-code">'+r.tokens.map(function(t,i){ return '<button class="bug-tok" data-i="'+i+'">'+esc(t)+'</button>'; }).join('')+'</div>'+
            '<div class="bug-hint">💡 '+esc(r.hint)+'</div>'+
            '<div class="bug-msg"></div>';
          host.querySelectorAll('.bug-tok').forEach(function(b){ b.addEventListener('click',function(){ tap(r,+b.getAttribute('data-i'),b); }); });
        }
        function tap(r,i,btn){
          var msg=host.querySelector('.bug-msg');
          if(i===r.bug){
            host.querySelectorAll('.bug-tok').forEach(function(b){ b.disabled=true; });
            btn.classList.add('fixed'); btn.textContent=r.fix;
            msg.innerHTML='<span class="ok">✅ Fixed! Now it runs.</span>'; ri++; setTimeout(render,1100);
          } else {
            btn.classList.add('miss'); setTimeout(function(){ btn.classList.remove('miss'); },500);
            hearts--; if(hearts<=0){ return setTimeout(lose,700); }
            msg.innerHTML='<span class="bad">❌ That part’s fine — look closer. '+hearts+' heart'+(hearts===1?'':'s')+' left.</span>';
          }
        }
        function win(){ host.innerHTML=immResult('🏅','Bug-free!', true, '<p>You squashed every bug! Real coders do this all day — even one wrong letter stops everything. The secret you learned: <b>bugs are normal, and finding them is the skill</b>.</p>'); bindRes(host, api, start); api.done(); }
        function lose(){ host.innerHTML=immResult('🐛','The bugs won this round', false, '<p>Sneaky! That’s real coding — sometimes you hunt a while. Watch for spelling slips, wrong signs (= vs ==), and missing letters. Try again!</p>'); bindRes(host, api, start); }
        start();
      }});
  }

  // RIGHT TOOL, RIGHT JOB (Mission 5) — match each job to the best language.
  function renderToolMatch(root, cfg, id){
    var LANGS=[{k:'python',l:'🐍 Python'},{k:'html',l:'🌐 HTML'},{k:'cpp',l:'⚡ C++'},{k:'sql',l:'🗄️ SQL'}];
    var JOBS = cfg.jobs || [
      {t:'Build a web page with words, pictures and buttons', best:'html', why:'HTML is the language of web pages.'},
      {t:'Teach a computer to recognize cats in photos (AI!)', best:'python', why:'Python is the favorite for AI and data.'},
      {t:'Make a super-fast racing video game', best:'cpp', why:'C++ is fast and powerful — great for games.'},
      {t:'Ask a giant list of millions of records a question', best:'sql', why:'SQL is built to ask questions of big piles of data.'},
      {t:'Build a smart chatbot that learns', best:'python', why:'AI work? Reach for Python.'},
      {t:'Make the home page of a brand-new website', best:'html', why:'A web page is built with HTML.'},
      {t:'Write the code inside a fast little robot', best:'cpp', why:'Speed matters in robots — C++ shines.'},
      {t:'Look up every student who scored over 90', best:'sql', why:'A question to big data = SQL.'}
    ];
    fsGame(root, cfg, id, { ico:'🧰', title: cfg.title||'Right Tool, Right Job',
      blurb:'A great builder picks the <b>right tool for the job</b>. A task pops up — choose the best coding language for it. Some are tricky! Get almost all of them right.',
      playLabel:'▶  Open the toolbox', wrapClass:'g-tool2',
      thanks: cfg.thanks || 'Python for AI, HTML for web, C++ for speed, SQL for data. 🧰',
      play:function(host, api){
        var queue, idx, score, NEED;
        function start(){ queue=shuffle(JOBS); idx=0; score=0; NEED=queue.length-1; render(); }
        function render(){
          if(idx>=queue.length) return finish();
          var j=queue[idx];
          host.innerHTML='<div class="tm-bg"></div>'+
            '<div class="tm-top"><span>Job '+(idx+1)+' / '+queue.length+'</span><span class="tm-score">⭐ '+score+'</span></div>'+
            '<div class="tm-job">🔧 '+esc(j.t)+'</div>'+
            '<div class="tm-instr">Which language fits best?</div>'+
            '<div class="tm-langs">'+LANGS.map(function(L){ return '<button class="tm-lang" data-k="'+L.k+'">'+L.l+'</button>'; }).join('')+'</div>'+
            '<div class="tm-fb" data-fb></div>';
          host.querySelectorAll('.tm-lang').forEach(function(b){ b.addEventListener('click',function(){ pick(j,b.getAttribute('data-k'),b); }); });
        }
        function pick(j,k,btn){
          host.querySelectorAll('.tm-lang').forEach(function(b){ b.disabled=true; if(b.getAttribute('data-k')===j.best) b.classList.add('right'); });
          var good=k===j.best, fb=host.querySelector('[data-fb]');
          if(good){ score++; fb.className='tm-fb ok'; fb.innerHTML='✅ '+esc(j.why); } else { btn.classList.add('wrong'); fb.className='tm-fb bad'; fb.innerHTML='❌ Not the best fit. '+esc(j.why); }
          fb.innerHTML+='<button class="imm-btn" data-next>'+(idx+1>=queue.length?'See your score →':'Next job →')+'</button>';
          fb.querySelector('[data-next]').addEventListener('click',function(){ idx++; render(); });
        }
        function finish(){
          if(score>=NEED) host.innerHTML=immResult('🧰','Master toolsmith! ('+score+'/'+queue.length+')', true, '<p>You matched almost every job to the right language. Remember: <b>Python</b> for AI/data, <b>HTML</b> for web pages, <b>C++</b> for fast games, <b>SQL</b> for asking big-data questions.</p>');
          else host.innerHTML=immResult('🧰','Keep practicing! ('+score+'/'+queue.length+')', false, '<p>A few tools didn’t fit. Web page → HTML, AI → Python, fast game → C++, big-data question → SQL. Try again!</p>');
  ...
  // PROMPT THE CODER (Mission 7) — sharpen a fuzzy prompt so the AI builds right.
  function renderPromptCoder(root, cfg, id){
    var ROUNDS = cfg.rounds || [
      { built:'You told the AI: “make a button.” It made a tiny gray button that does nothing. 😐', q:'What would help the AI most?', patches:[
        { l:'Say its color, size, label, AND what it does when clicked', ok:true, r:'Yes! Clear details = the AI builds exactly what you pictured.' },
        { l:'Just say “please” to be polite', ok:false, r:'Politeness is nice, but it doesn’t tell the AI what to build.' },
        { l:'Ask it to make ten buttons instead', ok:false, r:'Now you have ten vague buttons. Still no details!' } ] },
      { built:'You told the AI: “make it fun.” It added flashing rainbow chaos everywhere. 🌈🤪', q:'What would help most?', patches:[
        { l:'Describe what “fun” means here — like a bouncy animation when you score', ok:true, r:'Perfect — now “fun” is something the AI can actually build.' },
        { l:'Tell it to just try harder', ok:false, r:'“Try harder” isn’t a detail. The AI still has to guess.' },
        { l:'Say “make it super-duper fun”', ok:false, r:'More words, same fuzziness. Be specific!' } ] },
      { built:'You told the AI: “fix the game.” It changed random stuff and broke more. 🔧', q:'What would help most?', patches:[
        { l:'Tell it exactly what’s broken and what “fixed” should look like', ok:true, r:'Exactly — name the problem and the goal, and the AI nails it.' },
        { l:'Say “you broke it, bad AI!”', ok:false, r:'Blaming doesn’t help it understand. Describe the problem!' },
        { l:'Tell it to start completely over', ok:false, r:'That throws away the good parts. Be specific instead.' } ] }
    ];
    fsGame(root, cfg, id, { ico:'🎯', title: cfg.title||'Prompt the Coder',
      blurb:'AI can write code — but it doesn’t read minds! A <b>fuzzy prompt</b> makes it guess wrong. Each round, pick the detail that makes your prompt <b>clear</b>, so the AI builds what you actually want.',
      playLabel:'▶  Start prompting', wrapClass:'g-prp',
      thanks: cfg.thanks || 'Clear thinking is the new superpower. 🎯',
      play:function(host, api){
        var queue, idx, patience;
        function start(){ queue=ROUNDS.map(function(r){ return { built:r.built, q:r.q, patches:shuffle(r.patches) }; }); idx=0; patience=3; render(null); }
        function render(msg){
          if(idx>=queue.length) return win();
          var r=queue[idx];
          host.innerHTML='<div class="prp-bg"></div>'+
            '<div class="prp-top"><span>Prompt '+(idx+1)+' / '+queue.length+'</span><span class="prp-pat">'+'🎯'.repeat(patience)+'⚪'.repeat(3-patience)+'</span></div>'+
            '<div class="prp-screen">🤖 '+esc(r.built)+'</div>'+
            (msg?'<div class="prp-msg '+msg.cls+'">'+msg.txt+'</div>':'<div class="prp-msg"></div>')+
            '<div class="prp-q">'+esc(r.q)+'</div>'+
            '<div class="prp-patches">'+r.patches.map(function(p,i){ return '<button class="prp-patch" data-i="'+i+'">'+esc(p.l)+'</button>'; }).join('')+'</div>';
          host.querySelectorAll('.prp-patch').forEach(function(b){ b.addEventListener('click',function(){ pick(r,+b.getAttribute('data-i'),b); }); });
        }
        function pick(r,i,btn){
          var p=r.patches[i]; host.querySelectorAll('.prp-patch').forEach(function(b){ b.disabled=true; }); btn.classList.add(p.ok?'right':'wrong');
          setTimeout(function(){ if(p.ok){ idx++; render({cls:'ok',txt:'✅ '+p.r}); } else { patience--; if(patience<=0){ return lose(); } render({cls:'bad',txt:'😵 '+p.r}); } }, 950);
        }
        function win(){ host.innerHTML=immResult('🎯','Crystal-clear prompts!', true, '<p>You learned the new superpower: <b>clear thinking</b>. The AI types fast, but YOU decide what to build — and the clearer you describe it, the better it gets. That’s the whole skill.</p>'); bindRes(host, api, start); api.done(); }
        function lose(){ host.innerHTML=immResult('🌀','Too fuzzy!', false, '<p>Vague prompts make the AI guess — and guess wrong. The fix is always the same: say exactly what you want, in detail. Try again!</p>'); bindRes(host, api, start); }
        start();
      }});
  }

  // RUN THE AGENT (Mission 8) — drive the plan→do→check→fix loop to “shipped”.
  function renderAgentLoop(root, cfg, id){
    var ORDER=['plan','do','check','fix'];
    var STATIONS=[{k:'plan',l:'📋 Plan'},{k:'do',l:'⚙️ Do'},{k:'check',l:'🔍 Check'},{k:'fix',l:'🔧 Fix'}];
    var STEPS = cfg.steps || [
      { station:'plan', q:'Your agent is ready to build. What should it do FIRST?', win:'Plan first — a good agent figures out the steps before it starts.',
        o:[{t:'Make a clear plan of the steps',ok:true},{t:'Start typing random code and hope',ok:false},{t:'Ask you to build it instead',ok:false}] },
      { station:'do', q:'The agent wrote the code. What now?', win:'Always run it — “do” means try it, not just trust it.',
        o:[{t:'Run it to see if it actually works',ok:true},{t:'Ship it without checking',ok:false},{t:'Delete it and start over',ok:false}] },
      { station:'check', q:'Running it found a bug! What should the agent do?', win:'That’s the loop! Check → fix → check again until it works.',
        o:[{t:'Fix the bug, then run it again',ok:true},{t:'Pretend the bug isn’t there',ok:false},{t:'Give up',ok:false}] },
      { station:'fix', q:'It’s fixed and runs. But is it what YOU actually wanted?', win:'Right — the human checks the goal. The agent can’t decide that for you.',
        o:[{t:'You (the human) check it matches the goal',ok:true},{t:'The agent decides it’s good enough',ok:false},{t:'Nobody checks',ok:false}] },
      { station:'done', q:'It works AND matches your goal. Now what?', win:'You approve and ship. The agent did the steps; YOU made the call.',
        o:[{t:'Ship it — you give the final yes',ok:true},{t:'Keep looping forever',ok:false},{t:'Let the agent ship whatever it wants',ok:false}] }
    ];
    fsGame(root, cfg, id, { ico:'🤖', title: cfg.title||'Run the Agent',
      blurb:'An <b>agent</b> doesn’t just answer — it runs a loop: <b>plan → do → check → fix → repeat</b> — until the job is done. Drive the loop and make the calls the agent can’t. Get it all the way to “shipped!”',
      playLabel:'▶  Start the agent', wrapClass:'g-agent',
      thanks: cfg.thanks || 'The agent brings the speed; you bring the goal and the final yes. 🤖',
      play:function(host, api){
        var si;
        function start(){ si=0; render(null); }
        function render(msg){
          if(si>=STEPS.length) return win();
          var s=STEPS[si], prog=Math.round((si/STEPS.length)*100), here=ORDER.indexOf(s.station);
          var order=s.o.map(function(o,i){return i;}).sort(function(){return Math.random()-0.5;});
          host.innerHTML='<div class="al-bg"></div>'+
            '<div class="al-loop">'+STATIONS.map(function(st,k){ return '<span class="al-station'+(st.k===s.station?' on':'')+((here>=0&&k<here)?' past':'')+'">'+st.l+'</span>'; }).join('<span class="al-arrow">→</span>')+'</div>'+
            '<div class="al-progwrap"><div class="al-prog"><i style="width:'+prog+'%"></i></div><span class="al-proglbl">Build: '+prog+'%</span></div>'+
            (msg?'<div class="al-msg '+msg.cls+'">'+msg.txt+'</div>':'<div class="al-msg"></div>')+
            '<div class="al-q">🤖 '+esc(s.q)+'</div>'+
            '<div class="al-opts">'+order.map(function(oi){ return '<button class="al-opt" data-i="'+oi+'">'+esc(s.o[oi].t)+'</button>'; }).join('')+'</div>';
          host.querySelectorAll('.al-opt').forEach(function(b){ b.addEventListener('click',function(){ pick(s,+b.getAttribute('data-i'),b); }); });
        }
        function pick(s,i,btn){
          var o=s.o[i]; host.querySelectorAll('.al-opt').forEach(function(b){ b.disabled=true; }); btn.classList.add(o.ok?'right':'wrong');
          setTimeout(function(){ if(o.ok){ si++; render({cls:'ok',txt:'✅ '+s.win}); } else { render({cls:'bad',txt:'⚠️ Not quite — the agent needs the right call here. Try again.'}); } }, 900);
        }
        function win(){ host.innerHTML=immResult('🚀','Shipped!', true, '<p>You drove the whole agent loop: <b>plan, do, check, fix, repeat</b> — and made the calls the agent couldn’t. That’s exactly how building works now: the agent brings the speed, <b>you</b> bring the goal and the final yes.</p>'); bindRes(host, api, start); api.done(); }
        start();
      }});
  }

  // MANAGE THE TEAM (Mission 9) — send each job to the right specialist agent.
  function renderManageTeam(root, cfg, id){
    var AGENTS=[{k:'design',l:'🎨 Designer',sub:'makes it look good'},{k:'bug',l:'🐛 Bug-Fixer',sub:'hunts mistakes'},{k:'test',l:'🧪 Tester',sub:'makes sure it works'}];
    var JOBS = cfg.jobs || [
      {t:'The login screen looks plain and ugly', best:'design', why:'Looks → the Designer.'},
      {t:'The app crashes when you tap “save”', best:'bug', why:'A crash is a bug → the Bug-Fixer.'},
      {t:'Make sure the new feature didn’t break anything', best:'test', why:'Checking it still works → the Tester.'},
      {t:'The buttons are all different sizes and messy', best:'design', why:'Layout and looks → the Designer.'},
      {t:'A typo is making the score show wrong', best:'bug', why:'A mistake in the code → the Bug-Fixer.'},
      {t:'Try the game 100 times to catch any problems', best:'test', why:'Lots of careful trying → the Tester.'}
    ];
    fsGame(root, cfg, id, { ico:'🧑‍💼', title: cfg.title||'Manage the Team',
      blurb:'A modern builder directs a <b>team of AI agents</b>, each best at one thing. As jobs come in, send each one to the right agent — like a director running the show. Match almost all of them!',
      playLabel:'▶  Lead the team', wrapClass:'g-team',
      thanks: cfg.thanks || 'You don’t type every line — you steer a team of agents. 🧑‍💼',
      play:function(host, api){
        var queue, idx, score, NEED;
        function start(){ queue=shuffle(JOBS); idx=0; score=0; NEED=queue.length-1; render(); }
        function render(){
          if(idx>=queue.length) return finish();
          var j=queue[idx];
          host.innerHTML='<div class="mt-bg"></div>'+
            '<div class="mt-top"><span>Job '+(idx+1)+' / '+queue.length+'</span><span class="mt-score">⭐ '+score+'</span></div>'+
            '<div class="mt-job">📋 '+esc(j.t)+'</div>'+
            '<div class="mt-instr">Which agent should take it?</div>'+
            '<div class="mt-agents">'+AGENTS.map(function(a){ return '<button class="mt-agent" data-k="'+a.k+'"><b>'+a.l+'</b><small>'+esc(a.sub)+'</small></button>'; }).join('')+'</div>'+
            '<div class="mt-fb" data-fb></div>';
          host.querySelectorAll('.mt-agent').forEach(function(b){ b.addEventListener('click',function(){ pick(j,b.getAttribute('data-k'),b); }); });
        }
        function pick(j,k,btn){
          host.querySelectorAll('.mt-agent').forEach(function(b){ b.disabled=true; if(b.getAttribute('data-k')===j.best) b.classList.add('right'); });
          var good=k===j.best, fb=host.querySelector('[data-fb]');
          if(good){ score++; fb.className='mt-fb ok'; fb.innerHTML='✅ '+esc(j.why); } else { btn.classList.add('wrong'); fb.className='mt-fb bad'; fb.innerHTML='❌ Another agent fits better. '+esc(j.why); }
          fb.innerHTML+='<button class="imm-btn" data-next>'+(idx+1>=queue.length?'See your day →':'Next job →')+'</button>';
          fb.querySelector('[data-next]').addEventListener('click',function(){ idx++; render(); });
        }
        function finish(){
          if(score>=NEED) host.innerHTML=immResult('🏆','Great direction! ('+score+'/'+queue.length+')', true, '<p>You ran the team like a real lead builder — sending each job to the agent who does it best. That’s the modern job: not typing every line, but <b>steering a team of agents</b> toward great work.</p>');
          else host.innerHTML=immResult('🧑‍💼','The team got tangled ('+score+'/'+queue.length+')', false, '<p>A few jobs went to the wrong agent. Looks → Designer, crashes/typos → Bug-Fixer, “does it still work?” → Tester. Try again!</p>');
          bindRes(host, api, start); if(score>=NEED) api.done();
        }
        start();
      }});
