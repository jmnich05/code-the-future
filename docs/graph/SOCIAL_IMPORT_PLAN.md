...
3. Bind each asset to a consent record that states subject basis, allowed
   ...
   24 hours old when authorization is evaluated.
...
10. Only after a 72-hour window may the graph compare post performance; a promoted learning still requires at least three comparable executions per arm.
